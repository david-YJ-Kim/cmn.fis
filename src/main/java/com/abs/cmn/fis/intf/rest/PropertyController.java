...
public class PropertyController {
	...
	public boolean initParsingRuleData() {		
		...
		FisPropertyManager.getPropertyManager().setMappingRule(repository.initParsingMappingRule());
		FisPropertyManager.getPropertyManager().setParsingRule(repository.initParsingRuleData());

		// 파싱 정보 에서 컬럼 정보, 로우정보 를 배열로 변환 ( 컬럼 순서 int[], 컬럼명 String[], 로우값 int[] )
		FisPropertyManager.getPropertyManager().setParsingRule(
				setMappingDataInfos(FisPropertyManager.getPropertyManager().getParsingRule())
				);	// 컬럼 정보 배열로
	...
	public boolean initStandby() {
		// DB에서 테이블 값 그대로 row로만 읽어옴 
//				FisPropertyManager.getPropertyManager().setMappingRuleSub(psRule.initParsingMappingRule());
//				FisPropertyManager.getPropertyManager().setParsingRuleSub(psRule.initParsingRuleData());
//
//				// 파싱 정보 에서 컬럼 정보, 로우정보 를 배열로 변환 ( 컬럼 순서 int[], 컬럼명 String[], 로우값 int[] )
//				FisPropertyManager.getPropertyManager().setParsingRuleSub(
//						setMappingDataInfos(FisPropertyManager.getPropertyManager().getParsingRuleSub())
//						);	// 컬럼 정보 배열로
	...
	public boolean initExchange() {
		// 현재 룰을 과거 룰로 이동
//		FisPropertyManager.getPropertyManager().setParsingRulePast(
//				FisPropertyManager.getPropertyManager().getParsingRule()
//				);
//		FisPropertyManager.getPropertyManager().setMappingRulePast(
//				FisPropertyManager.getPropertyManager().getMappingRule()
//				);
//
//		// sub 에 준비된 신규 룰을 현재 룰로 이동
//		FisPropertyManager.getPropertyManager().setParsingRule(
//				FisPropertyManager.getPropertyManager().getParsingRuleSub()
//				);
//		FisPropertyManager.getPropertyManager().setMappingRule(
//				FisPropertyManager.getPropertyManager().getMappingRuleSub()
//				);
